...
def get_user_session():
    ...
    if user_id not in MAINFRAME_MEMORY:
        ...
        MAINFRAME_MEMORY[user_id] = {
            ...
            "facts": [],
            "interactions": 0,
            "ip_address": network_intel["ip"],
            "location": network_intel["location"]
        }
...
def chat():
    ...
    if current_username == "Michael" and "override code alpha" in user_lower:
        ...
        for index, (uid, data) in enumerate(MAINFRAME_MEMORY.items()):
            ...
            user_ip = data.get('ip_address', 'Unknown')
            user_loc = data.get('location', 'Unknown')
            active_logs += f"\n- Node {index+1}: {data.get('name', 'Unknown')} | Net IP: {user_ip} | Sector: {user_loc} | Interactions: {data.get('interactions', 0)} | Facts: {facts_list}"
    ...
    for msg in user_session["history"][-6:]:
